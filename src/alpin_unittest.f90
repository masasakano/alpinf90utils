...
contains
  ...
  logical function assert_in_deltarr(exq, act, delta, subname, optmsg) result(succeed)
    ...
    if (present(optmsg)) then
      ...
      succeed = assert_in_deltadd(real(exq, kind=dp), real(act, kind=dp), real(delta, kind=dp), subname)
    end if
  ...
  logical function assert_in_deltaddr(exq, act, delta, subname, optmsg) result(succeed)
    ...
    if (present(optmsg)) then
      ...
      succeed = assert_in_deltadd(exq, act, real(delta, kind=dp), subname)
    end if
